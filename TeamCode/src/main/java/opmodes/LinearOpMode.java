 ...
 * OpMode.
 *
 * Edited to make it much more flexible
 */
public abstract class LinearOpMode extends OpMode {
   ...
   * @deprecated The need for user code to synchronize with the loop() thread has been obviated by
   * improvements in the modern motor and servo controller implementations. Remaining uses of this
   * API are likely unncessarily wasting cycles. If a simple non-zero delay is required, the {@link
   * Thread#sleep(long) sleep()} method is a better choice. If one simply wants to allow other
   * threads to run, {@link #idle()} is a good choice.
   */
  @Deprecated
   ...
   *
   * @return whether the OpMode is currently active. If this returns false, you should break out of
   * the loop in your {@link #runOpMode()} method and return to its caller.
   * @see #runOpMode()
   * @see #isStarted()
   * @see #isStopRequested()
   */
      ...
      ThreadPool.logThreadLifeCycle("LinearOpMode main", new Runnable() {
        ...
        public void run() {
          ...
          try {
            ...
            RobotLog
                .d("LinearOpMode received an InterruptedException; shutting down this linear op mode");
            ...
            // best choice.
            RobotLog
                .d("LinearOpMode received a CancellationException; shutting down this linear op mode");
            ...
            TimestampedI2cData.suppressNewHealthWarningsWhile(new Runnable() {
              ...
              public void run() {
                if (telemetry instanceof TelemetryInternal) {
                  telemetry
                      .setMsTransmissionInterval(0); // will be reset the next time the opmode runs
